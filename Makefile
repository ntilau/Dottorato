...
# MS_BIB: Bibliography processor (optional, biber by default)
# MS_FILE: Main LaTeX source file (default: thesis)
MS_TEX      = pdflatex
MS_TEXFLAGS ?= -interaction=nonstopmode -file-line-error
MS_BIB      ?= biber
MS_BIB      := $(if $(strip $(MS_BIB)),$(strip $(MS_BIB)),biber)
...
presentation:
	...
	@if [ -n "$(PR_BIB)" ]; then cd presentation && $(PR_BIB) $(PR_FILE); fi
	cd presentation && $(PR_TEX) $(PR_TEXFLAGS) $(PR_FILE).tex
	cd presentation && $(PR_TEX) $(PR_TEXFLAGS) $(PR_FILE).tex
